  ...
  const memoTrack = useMemo(
    () => (
      <li className={className}>
        <button className="play-button" onClick={clickPlayHandler}>
          <FontAwesomeIcon icon={faPlay} />
        </button>
        <div className="before-title">
          {ordnum ? <p className="order-number">{ordnum}</p> : null}
          {image ? (
            <SquareImage className="track-image" radius="10px" src={image} />
          ) : null}
        </div>
        ...
        <div className="after-title">
          {album ? (
            <button onClick={clickAlbumHandler} className="track-album">
              {album.name}
            </button>
          ) : null}
          {duration ? (
            <p className="track-duration">{countTime(duration)}</p>
          ) : null}
        </div>
      </li>
    ),
...
const countTime = (s) => {
  ...
  time_string +=
    minutes < 10 ? ":" + (seconds < 10 ? "0" + seconds : seconds) : "";
  time_string += " min";
