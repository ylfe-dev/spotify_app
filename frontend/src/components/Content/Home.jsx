  ...
  return (
    <div className="home-featured app-tile">
      <div className="scroller">
          <h1>{message}</h1>
          <div className="tiles">
              {playlists.items.map(playlist => <PlaylistTile playlist={playlist}/>)}
          </div>
      </div>
    </div>
  );
...
const  PlaylistTile = ({playlist}) => {
  ...
  return (
    <article className="playlist-tile" onClick={handlePlaylistClick}>
      {playlist.images[0] ? <SquareImage className="tile-image" src={playlist.images[0].url} size={150} radius={10} /> : null }
