...
import { faRecordVinyl, faEye, faMusic } from '@fortawesome/free-solid-svg-icons'
		...
		<div className="app-artist">
			<Suspense fallback={<Spinner />}>
				<div className="artist-overview app-tile">
					<ArtistHeader fetcher={artist_promise} fetcherAlbums={artistAlbums_promise}/>
	...
	return artist ? (
		<header>
			...
			<p className="artist-details">
				...
				<span><FontAwesomeIcon icon={faRecordVinyl} />{albums.total}</span>
			</p>
					...
					return (
						<div key={year.full}>
							...
							<div className="albums-year">
								{year.albums.map(album => <ArtistAlbum key={album.id} info={album} />)}
							</div>
						</div>
					)
				})}
...
const ArtistAlbum = ({info}) => {
	...
	return (info && info.name) ? (
		<article className="artist-album" onClick={clickAlbumHandler}>
			{info.images ? <SquareImage className="album-image" size={120} radius={10} src={info.images[0].url}/> : null}
			<h3>{info.name}</h3>
			<p className="album-artists">
				{info.artists.map(artist => <span key={artist.id} onClick={(ev)=>clickArtistHandler(ev, artist.id)}>{artist.name}</span>)}
			</p>
			<p className="album-details">
				<span><FontAwesomeIcon icon={faRecordVinyl} />{info.album_type}</span>
				<span><FontAwesomeIcon icon={faMusic} />{info.total_tracks}</span>
			</p>
		</article>
	) : null
}